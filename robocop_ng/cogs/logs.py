...
class Logs(Cog):
    ...
    async def on_message_edit(self, before, after):
        ...
        # Prepare embed msg
        embed = discord.Embed(
            color=discord.Color.dark_gray(), title="📝 Message Edit in {after.channel.mention}", description=f"<@{after.author.id}>  ({after.author.id}) [[Jump]({after.jump_url})]", timestamp=datetime.datetime.now()
        )
        embed.set_footer(text="Dishwasher")
